...
describe(`getDataFromExcelFileU8Arr`, function() {
    ...
    it(`should return ${JSON.stringify(o[k].out)} when input ${JSON.stringify(o[k].in)}`, async function() {
        let k = 0
        let r = await getDataFromExcelFileU8Arr(u8a, o[k].in)
        r = r[0].data[1]
...
`
    ...
    it(`should omit keys for blank cells in fmt='ltdt' (not fill with '')`, async function() {
        //ltdt模式空cell為略鍵而非給空字串, 為既有sheet_to_json預設語意, 引擎抽換後亦須保持
        let ub = await getExcelU8ArrFromData([['h1', 'h2', 'h3'], ['a', null, 'c']])
        let r1 = await getDataFromExcelFileU8Arr(ub, { fmt: 'ltdt' })
        assert.strict.deepStrictEqual(r1[0].data, [{ h1: 'a', h3: 'c' }])
        let r2 = await getDataFromExcelFileU8Arr(ub, { fmt: 'ltdt', valueToString: false })
        assert.strict.deepStrictEqual(r2[0].data, [{ h1: 'a', h3: 'c' }])
    })

    it(`should keep '' value (not omit key) for empty-string cells in fmt='ltdt'`, async function() {
        //空字串cell是實體cell, 與空cell(略鍵)不同, 鍵須存在且值為''
        let ub = await getExcelU8ArrFromData([['h1', 'h2'], ['a', '']])
        let r = await getDataFromExcelFileU8Arr(ub, { fmt: 'ltdt' })
        assert.strict.deepStrictEqual(r[0].data, [{ h1: 'a', h2: '' }])
    })

    it(`should return error object when u8a is not a valid xlsx binary`, async function() {
        //SheetJS舊實作會將垃圾bytes靜默fallback成csv解析而回假成功, 改用hucre後正確回報錯誤
        let r = await getDataFromExcelFileU8Arr(new Uint8Array([1, 2, 3]), { fmt: 'array' })
        assert.strict.deepStrictEqual(r.error, 'can not read data from u8a')
    })

    it(`should return error object when opt.fmt is invalid`, async function() {
        let r = await getDataFromExcelFileU8Arr(u8a, { fmt: 'xxx' })
        assert.strict.deepStrictEqual(r.error, `opt.fmt is not one of 'ltdt', 'csv', 'array'`)
    })
