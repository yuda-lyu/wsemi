...
/**
 * 將command + args轉為Windows安全的spawn參數
 * 策略優先順序:
 *   1. .exe → 直接spawn
 ...
 * @returns {Object} 回傳spawn參數物件, 內含file, args, opt
 */
...
function execCliOnce(command, args = [], opt = {}) {
    ...
    if (envExtra !== undefined) {
        if (process.platform === 'win32') {
            //Windows環境變數大小寫不敏感但JS物件敏感, 須先移除大小寫不同之同名既有鍵,
            //否則呼叫端傳Path而process.env為PATH時兩鍵並存且Windows取原值, 注入靜默失效
