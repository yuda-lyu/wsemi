...
import fsDownloadFileCore from './fsDownloadFileCore.mjs'


/**
 * 後端nodejs下載網址成為檔案
 *
 * 下載內容先寫入暫存檔`${fpOut}.download`，傳輸完成後才改名為fpOut，故中途失敗時fpOut不會殘留截斷檔，下次呼叫自然重新下載。回應帶content-length且未經內容編碼(gzip等會自動解壓，該標頭為壓縮後大小故不核對)時，改名前會核對暫存檔大小。任何失敗(非2xx回應、連線被拒、下載中途斷線、大小不符、寫檔錯誤)皆reject字串`failed to download url[urlIn] to file[fpOut]: 原因`，並清除暫存檔。同一fpOut之併發下載由呼叫端自行避免
 *
 ...
 *
 */
async function fsDownloadFile(urlIn, fpOut) {
    return fsDownloadFileCore(urlIn, fpOut, { fs, Readable, pipeline })
}


export default fsDownloadFile
