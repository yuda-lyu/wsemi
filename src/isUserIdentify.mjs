...
function isUserIdentify(v) {
    ...
    if (isNaN(v.substring(1, 1 + 9)) || (!/^[A-Z]$/.test(v.substring(0, 0 + 1)))) {
    ...
    v = (idHeader.indexOf(v.substring(0, 1)) + 10) + '' + v.substring(1, 1 + 9)
    ...
    let s = parseInt(v.substring(0, 0 + 1)) +
            parseInt(v.substring(1, 1 + 1)) * 9 +
            parseInt(v.substring(2, 2 + 1)) * 8 +
            parseInt(v.substring(3, 3 + 1)) * 7 +
            parseInt(v.substring(4, 4 + 1)) * 6 +
            parseInt(v.substring(5, 5 + 1)) * 5 +
            parseInt(v.substring(6, 6 + 1)) * 4 +
            parseInt(v.substring(7, 7 + 1)) * 3 +
            parseInt(v.substring(8, 8 + 1)) * 2 +
            parseInt(v.substring(9, 9 + 1))

    //檢查碼
    // let checkNum = parseInt(v.substr(10, 1))
    let checkNum = parseInt(v.substring(10, 10 + 1))
