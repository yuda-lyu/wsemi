 ...
 * @memberOf wsemi
 * @returns {Object} 回傳事件物件，可呼叫函數on、set、get、getProxy、clear、remove。on為監聽事件，需自行監聽message與error事件。set為加入待執行函數，函數結束回傳欲快取的值，set傳入參數依序為key與快取物件，key為唯一識別字串，可使用函數加上輸入參數作為key，因考慮輸入參數可能為大量數據會有效能問題，由開發者自行決定key，而快取物件需設定欄位fun為待執行的非同步函數、inputs為待執行函數fun的傳入參數組、timeExpired為過期時間整數，單位為毫秒ms，預設5000、timeFrom為快取時間之起算點，可選'start'或'end'，'start'為自函數開始執行時起算，'end'為自函數執行完畢時起算，預設'start'、useCacheWhenError為函數執行失敗時是否仍寫入快取，預設false，失敗不寫入快取，錯誤會拋給呼叫端(含等待中之併發呼叫)且下次get會重新執行，故呼叫端需自行catch，若設定true則失敗仍寫入快取，此時快取值為undefined並保留整個timeExpired，呼叫端不會收到錯誤而是取得undefined、useCloneDeep為取值時是否複製快取值後才回傳，預設true，因快取值若為物件則可能受外部調用而被修改到快取區的記憶體，若快取值為大量數據可設定false以節省複製開銷，但呼叫端須自行確保不修改所取得之值。get為依照key取得目前快取值，若該key之函數執行中則共用執行中之promise，待其執行完畢即一併取得結果，不另行輪詢，get傳入參數依序為key與設定物件，設定物件之欄位useCloneDeep可覆寫set時之設定，未指定時沿用set之設定。getProxy為合併set與get功能，直接set註冊待執行函數與取值，傳入參數同set，且會一併傳給get，回傳同get。update為強制更新key所屬快取值，同時也會更新該快取之時間至當前，於函數執行中呼叫時以update之值為準，不被執行結果覆寫。clear為清除key所屬快取的是否執行標記，使該快取視為需重新執行函數取值，於函數執行中呼叫亦有效，執行中之呼叫仍取得本次結果，下次get才重新執行。remove為直接清除key所屬快取，清除後用set重設。事件以timer脫勾派發且事件物件為evem之safe型，監聽器拋錯或async reject不會使行程崩潰，會改以error事件回報{ fun: 'listener', name, msg, args }；故error事件同時承載本模組自身之錯誤(fun為'get'等且帶key)與監聽器錯誤(fun為'listener'且無key)，監聽error時請先以fun欄位分流
 * @example
 *
 ...
 *         //useCacheWhenError: 預設false則失敗不寫入快取, 錯誤拋給執行者與等待中之併發呼叫, 下次get重新執行; 設true則失敗仍寫入快取(值為undefined)並保留整個timeExpired, 呼叫端取得undefined而非收到錯誤; 兩者error事件皆發出
 *         let oc = cache()
 *
 *         let errs = []
 *         oc.on('error', function(msg) {
 ...
 *         }
 *
 *         oc.set('fun', { fun, inputs: [], timeExpired: 30000 }) //預設useCacheWhenError=false
 *
 ...
 *
 *         //useCacheWhenError=true時, 失敗不拋錯而是取得undefined, 且快取整個timeExpired故不重新執行
 *         let m = 0
 *         let funErr = async () => {
 *             m++
 *             throw new Error('always down')
 *         }
 *         oc.set('funErr', { fun: funErr, inputs: [], timeExpired: 30000, useCacheWhenError: true })
 *         let c1 = await oc.get('funErr').then((v) => 'ok:' + v).catch((e) => 'err:' + e.message)
 *         let c2 = await oc.get('funErr').then((v) => 'ok:' + v).catch((e) => 'err:' + e.message)
 *
 *         await new Promise((resolve) => setTimeout(resolve, 50)) //error事件以timer脫勾發送, 稍待
 *
 *         return { rs: [r1, r2, r3, r4, r5], n, cs: [c1, c2], m, errs }
 *     }
 *     console.log('test6')
 *     let r6 = await test6()
 *     console.log(JSON.stringify(r6))
 *     // test6
 *     // {"rs":["err:down1","err:down1","err:down2","ok:up3","ok:up3"],"n":3,"cs":["ok:undefined","ok:undefined"],"m":1,"errs":["fun:down1","fun:down2","funErr:always down"]}
 *
 ...
 */
function cache() {
    let ev = evem({ type: 'safe' }) //事件以timer脫勾派發, 監聽器出錯不得殺行程, 由evem預設政策重發error事件
    let data = {} //快取資料
    ...
    function set(key, opt = {}) {
        ...
        //useCacheWhenError
        let useCacheWhenError = loGet(opt, 'useCacheWhenError')
        if (!isbol(useCacheWhenError)) {
            useCacheWhenError = false
        }
        ...
        emit('message', { fun: 'set', key, timeExpired, timeFrom, useCacheWhenError, useCloneDeep })
                ...
                let run = async () => {
                    try {
                        ...
                        try {
                            ...
                            if (!d.useCacheWhenError) {
                                //失敗不寫入快取: 值與時間不動, 下次get重新執行, 錯誤拋給執行者與等待中之併發呼叫
                                if (d.gen === gen) {
                                    d.needExec = true
                                }
                                throw err
                            }
                            v = undefined //失敗仍寫入快取: 值為undefined並保留整個timeExpired
                        }
                        if (d.gen === gen) {
                            d.value = v
